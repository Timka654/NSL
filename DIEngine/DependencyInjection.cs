...
namespace DIEngine
{
    public class DependencyInjection
    {
        ...
        private List<KeyValuePair<Type, object>> waitBuildObjectMap = new List<KeyValuePair<Type, object>>();
        ...
        public void Attach(object obj, bool process = false, bool waitBuild = false)
        {
            if (process)
            {
                if (waitBuild)
                {
                    waitBuildObjectMap.Add(new KeyValuePair<Type, object>(obj.GetType(), obj));
                    return;
                }
                ProcessObject(obj);
            }
        ...
        public void Attach<T>(bool waitBuild = false)
        {
            ...
            if (waitBuild)
            {
                waitBuildObjectMap.Add(new KeyValuePair<Type, object>(typeof(T), obj));
                return;
            }
