...
namespace NSL.ASPNET.Mvc
{
    public class ControllerResults
    {
        ...
        public static object FormatModelState(ModelStateDictionary modelState)
            => modelState.ToDictionary(
                x => x.Key,
                x => x.Value.Errors.Select(b => new HttpResponseErrorModel(b.ErrorMessage, Array.Empty<string>())).ToArray()
                );
    }
}
