...
namespace NSL.Generators.HttpEndPointGenerator.Core
{
    ...
    internal class HttpEndPointGenerator : IIncrementalGenerator
    {
        ...
        private void ProcessHttpEndPoint(SourceProductionContext sourceContext, GeneratorSyntaxContext context, TypeDeclarationSyntax type)
        {
            ...
            {
                "NSL.HttpClient",
                "NSL.HttpClient.HttpContent",
                "NSL.HttpClient.Models",
                "NSL.HttpClient.Validators"
            };

            typeSymb.GetAttributes()
                .Where(x => x.AttributeClass.Name.Equals(IncludeUsingAttributeFullName))
                .SelectMany(x => x.ConstructorArguments.FirstOrDefault().Values)
                .Select(x=> x.Value as string)
                .Where(x => !string.IsNullOrEmpty(x))
                .ToList()
                .ForEach(x => usings.Add(x));

            var excludeUsings = typeSymb.GetAttributes()
                .Where(x => x.AttributeClass.Name.Equals(ExcludeUsingAttributeFullName))
                .SelectMany(x => x.ConstructorArguments.FirstOrDefault().Values)
                .Select(x => x.Value as string)
                .Where(x => !string.IsNullOrEmpty(x))
                .ToList();
            ...
            {
                codeBuilder.AppendLine($"Auto Generated by NSL HttpEndPoint Generator. Please don't change this file");
            });
                    ...
                    foreach (var item in members)
                    {
                        ...
                        if (url == default)
                        {
                            ...
                            foreach (var templ in templates)
                            {
                                ...
                                {
                                    b.AppendSummaryLine($"Generate for <see cref=\"{ms.GetMethodSeeCRef()}\"/>");
                                });
#if DEBUG
                                //GenDebug.Break();
#endif
...
#pragma warning restore RS1035 // Не использовать API, запрещенные для анализаторов
            }, usings, beforeClassDef: builder => builder.AppendSummary(b =>
            {
                b.AppendSummaryLine($"Generate for <see cref=\"{typeSymb.GetTypeSeeCRef()}\"/>");
            }), exceptUsings: excludeUsings);
...
#pragma warning restore RS1035 // Do not use APIs banned for analyzers
        ...
        private readonly string IncludeUsingAttributeFullName = typeof(HttpEndPointIncludeUsingAttribute).Name;
        private readonly string ExcludeUsingAttributeFullName = typeof(HttpEndPointExcludeUsingAttribute).Name;
