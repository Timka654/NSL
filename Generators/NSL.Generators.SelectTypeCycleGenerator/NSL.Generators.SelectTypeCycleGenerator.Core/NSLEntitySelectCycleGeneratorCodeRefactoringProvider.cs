...
namespace NSL.Generators.SelectTypeCycleGenerator.Core
{
    ...
    internal class NSLEntitySelectCycleGeneratorCodeRefactoringProvider : IIncrementalGenerator
    {
        ...
        private static void Execute(Compilation compilation, ImmutableArray<ClassDeclarationSyntax> classes, SourceProductionContext context)
        {
            ...
            foreach (var classSyntax in classes.Distinct())
            {
                ...
                // DTO generation
                if (metadata.DtoEnabled)
                {
                    foreach (var model in metadata.SelectModels)
                    {
                        var dtoCode = CycleDtoGenerator.GenerateCycleDto(classSymbol, metadata, model);
                        if (dtoCode == null) continue;

                        var className = CycleDtoGenerator.BuildCycleDtoName(classSymbol.Name, model, metadata.DtoSuffix);
                        context.AddSource($"{className}.cycledto.g.cs", dtoCode);
                    }
                }
            }
        }
        private static CycleClassMetadata ParseMetadata(INamedTypeSymbol classSymbol)
        {
            ...
            foreach (var attr in classSymbol.GetAttributes())
            {
                ...
                if (attrName == "SqlCycleSelectAttribute" || attrName == "SqlCycleSelect")
                {
                    ...
                    // Read Dto and DtoSuffix from named arguments
                    foreach (var namedArg in attr.NamedArguments)
                    {
                        if (namedArg.Key == "Dto" && namedArg.Value.Value is bool dtoEnabled)
                            metadata.DtoEnabled |= dtoEnabled;
                        else if (namedArg.Key == "DtoSuffix" && namedArg.Value.Value is string dtoSuffix)
                            metadata.DtoSuffix = dtoSuffix;
                    }
                }
...
SELECT * FROM cte"";");
    ...
    internal class CycleClassMetadata
    {
        ...
        public List<CyclePaginationMetadata> Paginations { get; set; } = new List<CyclePaginationMetadata>();
        public List<CycleOrderByMetadata> OrderBys { get; set; } = new List<CycleOrderByMetadata>();

        public bool DtoEnabled { get; set; }
    ...
    internal static partial class CycleDtoGenerator
    {
        internal static string GenerateCycleDto(INamedTypeSymbol classSymbol, CycleClassMetadata metadata, string modelName)
        {
            var proxyNames = new HashSet<string>(metadata.Proxies.Select(p => p.PropertyName));

            var includedMembers = new HashSet<string>(metadata.Members
                .Where(m => (m.Models == null || m.Models.Length == 0 || m.Models.Contains(modelName))
                         && !proxyNames.Contains(m.PropertyName))
            ...
            sb.AppendLine($"    /// <summary>DTO generated for <see cref=\"{metadata.ClassName}\"/> model \"{modelName}\"</summary>");
            sb.AppendLine($"    public class {BuildCycleDtoName(metadata.ClassName, modelName, metadata.DtoSuffix)}");
            sb.AppendLine("    {");

            foreach (var member in classSymbol.GetMembers().OfType<IPropertySymbol>()
                .Where(p => includedMembers.Contains(p.Name)))
            {
        ...
        internal static string BuildCycleDtoName(string typeName, string modelName, string suffix)
        {
