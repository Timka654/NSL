...
{
    public class BinaryDateTime : BasicType
    {
        ...
        {
            value = (ulong)((DateTime)value - DateTime.MinValue).TotalMilliseconds;
            fixed (byte* b = buffer)
        ...
        public override unsafe object GetValue(ref byte[] buffer, int offset)
        {
            fixed (byte* pbyte = &buffer[offset])
                return DateTime.MinValue.AddMilliseconds((ulong)((*pbyte) | (*(pbyte + 1) << 8) | (*(pbyte + 2) << 16) | (*(pbyte + 3) << 24) | (*(pbyte + 3) << 32) | (*(pbyte + 3) << 40) | (*(pbyte + 3) << 48) | (*(pbyte + 3) << 56)));
        }
    }
}
