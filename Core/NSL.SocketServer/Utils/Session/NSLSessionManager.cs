...
namespace NSL.SocketServer.Utils.Session
{
    public class NSLSessionManager<TClient> : IDisposable where TClient : IServerNetworkClient
    {
        ...
        private readonly ConcurrentDictionary<string, NSLServerSessionInfo<TClient>> sessionStorage = new ConcurrentDictionary<string, NSLServerSessionInfo<TClient>>();
        private readonly ConcurrentQueue<NSLServerSessionInfo<TClient>> waitCloseQueue = new ConcurrentQueue<NSLServerSessionInfo<TClient>>();
        private readonly CancellationTokenSource removeSessionCycleCTS;
        ...
        public bool IsValidSession(TClient client)
        {
            ...
            return client.GetSessionInfo() != null;
        }
        ...
        public async Task<NSLRecoverySessionResult> TryRecovery(TClient client, string session, string[] keys)
        {
            if (!sessionStorage.TryGetValue(session, out var oldSession)
                ...
                || oldSession.DisconnectTime < DateTime.UtcNow)
            {
