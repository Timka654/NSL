...
{
    public class ITestStruct<T> where T : new()
    {
        ...
        public virtual void bsSerializeAction(Stopwatch sw)
        {
            if (serializerWriteBuffer == null)
            {
                ...
                var sbuf = new SocketCore.Utils.Buffer.OutputPacketBuffer();
                Utils.bs.Serialize(sbuf, normalValue);
                serializerWriteBuffer = sbuf.ToArray();
            }
            sw.Start();
            Utils.bs.Serialize(new SocketCore.Utils.Buffer.OutputPacketBuffer(), normalValue, "default");
            sw.Stop();
        }
        ...
        {
            int offset = 0;
            var nbuf = new InputPacketBuffer(serializerWriteBuffer);
            if (deserializedValue == null)
            {

                deserializedValue = Utils.bs.Deserialize<T>(nbuf, "default");
                offset = 0;
            }

            sw.Start();
            Utils.bs.Deserialize<T>(nbuf, "default");
            sw.Stop();
            nbuf.Position = 7;
        }
